...
class Oterm:
    ...
    def __str__(self):
        return f"Oterm({self.id} {self.name} - alt: {self.alt_names} - isa: {self.isaList} - obsolete: {self.obsolete} - update: {self.update_instructions} )"
...
class Ordo_Parser:
    ...
    # - - - - - - - - - - - - - - - - - - 
        ...
        return Term(id, o_term.name, list(o_term.alt_names), list(parent_set), self.abbrev)


    # - - - - - - - - - - - - - - - - - - 
    # INTERFACE
    # - - - - - - - - - - - - - - - - - - 
    def export_as_tsv(self, include_synonyms=True):
    # - - - - - - - - - - - - - - - - - - 
        filename = f"terminologies/{self.abbrev}.tsv"
    ...
    # - - - - - - - - - - - - - - - - - - 
        ...
        for cl in classes:
            ...
            for node in cl.xpath("efo:alternative_term", namespaces=ns):
                if node.text != oterm.name: oterm.alt_names.add(node.text)
